...
{
    public function testUserCanPayWithInvoicePayment()
    {
    ...
    public function testUserCanRefundWithInvoicePayment()
    {
        $transaction = Transaction::factory()->create(['amount' => 3, 'payment_type_id' => 3]);
        ...
        $this->assertEquals('invoice', $response->json()['data']['payment_type']);
    ...
    public function testUserMustPayWithTheSameCurrencyThatUserBeforeInTheSameTransaction(){
        ...
        Transaction::factory()->create(['amount' => 3, 'currency' => 'USD', 'payment_type_id' => 3, 'client_key' => $data['client_key']]);
    ...
    public function testUserMustConfigurePaymentTypesBeforePay(){
        ...
        Transaction::factory()->create(['amount' => 3,'payment_type_id' => 3]);
        $response = $this->post('/api/pay', $this->preparePaymentData());
        $response->assertStatus(422);
        $response->assertJsonStructure(
            [
